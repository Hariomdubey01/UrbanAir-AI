  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-forest-800/10 dark:border-white/[0.08]">
        <div>
          <span className="text-xs font-semibold uppercase tracking-wider text-emerald-600 dark:text-emerald-400">Environmental Dashboard</span>
          <h1 className="text-2xl sm:text-3xl font-semibold text-forest-800 dark:text-white tracking-tight mt-0.5">
            How is the air in your city today?
          </h1>
          <p className="text-muted text-xs">Real-time telemetry and grounded AI explanation for {selectedCity.name}</p>
        </div>

        <button
          onClick={() => setSearchOpen(true)}
          className="flex items-center gap-2.5 px-4 py-2.5 rounded-xl bg-white dark:bg-[#0D1B18] hover:bg-ivory-200 dark:hover:bg-forest-900 text-forest-800 dark:text-white text-xs font-semibold border border-forest-800/15 dark:border-white/[0.08] transition-all shadow-sm"
        >
          <MapPin className="w-4 h-4 text-emerald-500" />
          <span>Location: <strong>{selectedCity.name}, {selectedCity.country}</strong></span>
          <Search className="w-3.5 h-3.5 text-muted ml-1" />
        </button>
      </div>
      ...
      {loading || !airQuality ? (
        <div className="p-12 text-center rounded-[20px] bg-white dark:bg-[#0D1B18] border border-forest-800/10 dark:border-white/[0.08] space-y-2">
          ...
          <p className="text-xs font-semibold text-muted">Fetching environmental metrics for {selectedCity.name}...</p>
        </div>
      ) : (
        <div className="space-y-10">
          ...
          <div className="space-y-3">
            <h2 className="text-lg font-semibold text-forest-800 dark:text-white tracking-tight">What's in the air?</h2>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-lg font-semibold text-forest-800 dark:text-white tracking-tight">UrbanAir AI Insight</h2>
              </div>
              {aiLoading && (
                <span className="text-xs text-ai-500 font-semibold flex items-center gap-1">
